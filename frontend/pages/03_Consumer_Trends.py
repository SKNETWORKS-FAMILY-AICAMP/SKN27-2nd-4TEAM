...
#     "metric": ["OrderCount", "CouponUsed", "Cashback", "Satisfaction"],
#     "churn": [2.82, 1.68, 16.0, 3.1],
#     "retain": [3.05, 1.77, 18.0, 3.8],
#     "churn_event": ["주문 횟수가 적을수록 이탈률 급증", "할인 없이는 구매 안하는 체리피커 성향", "리워드 부족이 체감 이탈의 주 원인", "전반적인 서비스 경험 불만족"],
#     "retain_event": ["주기적인 방문 및 구매 유지", "쿠폰 외에도 자연스러운 구매 발생", "충분한 리워드로 강력한 락인(Lock-in)", "높은 서비스 만족도"]
# })
...
#     st.markdown("<p style='color: #64748B; font-size: 14px; margin-bottom: 24px;'>2020~2026 통계청·한국은행 트렌드와 고객 이탈 지표 연결</p>", unsafe_allow_html=True)
# with head_col2:
#     global_year_filter = st.selectbox("기간 설정", year_options, label_visibility="collapsed")

# # --- 6. 연결 다이어그램 ---
# connections = [
#     {"trend": "소비 위축 · 지출 감소", "icon": "📉", "trendColor": "#D97A7A", "trendBg": "#FDEAEA", "metric": "OrderCount 감소", "metricDesc": "이탈 2.82회 vs 유지 3.05회", "metricColor": "#C45A5A", "metricBg": "#FDEAEA"},
#     {"trend": "구매 공백 증가", "icon": "🛒", "trendColor": "#E6A050", "trendBg": "#FDF1E3", "metric": "DaySinceLastOrder 증가", "metricDesc": "이탈 3.24일 vs 유지 4.81일", "metricColor": "#B87535", "metricBg": "#FDF1E3"},
#     {"trend": "가성비 소비 증가", "icon": "🏷️", "trendColor": "#4A6FA5", "trendBg": "#EEF2F8", "metric": "CouponUsed 차이", "metricDesc": "이탈 1.68개 vs 유지 1.77개", "metricColor": "#3A5A8C", "metricBg": "#EEF2F8"},
#     {"trend": "모바일 쇼핑 확대", "icon": "📱", "trendColor": "#5FAD56", "trendBg": "#EDF7EC", "metric": "모바일 비중 급증", "metricDesc": "2020년 69.0% → 2026년 78.2%", "metricColor": "#3D7A3C", "metricBg": "#EDF7EC"},
# ]

# with st.container(border=True):
#     st.markdown("<p style='font-weight: 700; font-size: 16px; color: #1E293B; margin-bottom: 2px;'>소비 트렌드 → 이커머스 데이터 연결</p>", unsafe_allow_html=True)
#     st.markdown("<p style='font-size: 12px; color: #94A3B8; margin-bottom: 20px;'>거시적 트렌드 변화가 고객 지표에 미치는 영향</p>", unsafe_allow_html=True)
    
#     for c in connections:
#         col1, col_arrow, col2 = st.columns([1, 0.1, 1])
#         with col1:
#             st.markdown(f"""
#                 <div style="background-color: {c['trendBg']}; border: 1px solid {c['trendColor']}35; padding: 12px; border-radius: 10px; display: flex; align-items: center; gap: 12px;">
#                     <div style="background-color: {c['trendColor']}25; width: 30px; height: 30px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 14px;">{c['icon']}</div>
#                     <div>
#                         <p style="margin: 0; font-size: 10px; color: #94A3B8;">한국 트렌드</p>
#                         <p style="margin: 0; font-size: 13px; font-weight: 700; color: #334155;">{c['trend']}</p>
#                     </div>
#                 </div>
#             """, unsafe_allow_html=True)
#         with col_arrow:
#             st.markdown(f"<div style='text-align: center; color: {c['trendColor']}; font-size: 20px; padding-top: 10px;'>→</div>", unsafe_allow_html=True)
#         with col2:
#             st.markdown(f"""
#                 <div style="background-color: {c['metricBg']}; border: 1px solid {c['metricColor']}35; padding: 12px; border-radius: 10px;">
#                     <p style="margin: 0; font-size: 13px; font-weight: 800; color: {c['metricColor']};">{c['metric']}</p>
#                     <p style="margin: 0; font-size: 11px; color: #64748B;">{c['metricDesc']}</p>
#                 </div>
#             """, unsafe_allow_html=True)
#         st.write("")
...
#             <p style='font-weight: 700; font-size: 15px; color: #1E293B; margin-bottom: 0;'>
#                 온라인 거래액 & 모바일 비중 <span style='color: #94A3B8; font-size: 13px; font-weight: 400;'>ⓘ</span>
#             </p>
#             <div class="tooltip-text">
#                 <b style='color:#60A5FA;'>📊 그래프 설명:</b> 전체 온라인 거래 규모와 모바일 의존도입니다.<br><br>
#                 <b style='color:#34D399;'>💡 해석 방법:</b> 파란 면적이 커지면 시장 성장, 초록 선이 올라가면 모바일 의존도 증가입니다. <b>더 진한 초록색 동그라미</b>에 마우스를 올려 주요 이슈를 확인하세요.
#             </div>
#         </div>
#     """, unsafe_allow_html=True)
...
#         x=df_filtered[x_col], y=df_filtered['amount'], name='거래액(조원)', fill='tozeroy', line_color='#4A6FA5', yaxis='y1',
#         hovertemplate="거래액: %{y:,.1f}조원<extra></extra>"
#     ))
    
#     # 🚨 이벤트 유무에 따라 모바일(초록색 선) 마커 색상 및 크기 동적 할당
#     mobile_colors = ['#14532D' if e != "" else '#5FAD56' for e in df_filtered['event']] # 진한 초록 vs 기본 초록
#     mobile_sizes = [12 if e != "" else 6 for e in df_filtered['event']]
    
#     fig1.add_trace(go.Scatter(
#         x=df_filtered[x_col], y=df_filtered['mobile'], name='모바일(%)', mode='lines+markers',
#         line=dict(color='#5FAD56', width=2),
#         marker=dict(color=mobile_colors, size=mobile_sizes, line=dict(color='white', width=1)),
#         customdata=df_filtered['event'], hovertemplate="모바일: %{y:.1f}%%{customdata}<extra></extra>", yaxis='y2'
#     ))
    
#     fig1.update_layout(
#         height=320, margin=dict(l=0, r=0, t=20, b=0), hovermode="x unified",
#         yaxis=dict(title="거래액(조원)", range=[130, 310]), yaxis2=dict(title="비중(%)", overlaying='y', side='right', range=[65, 80]),
...
#         <div class="title-tooltip">
#             <p style='font-weight: 700; font-size: 15px; color: #1E293B; margin-bottom: 0;'>
#                 소비자심리지수 (CCSI) 추이 <span style='color: #94A3B8; font-size: 13px; font-weight: 400;'>ⓘ</span>
#             </p>
#             <div class="tooltip-text">
#                 <b style='color:#F472B6;'>📊 그래프 설명:</b> 소비자들의 체감 심리 지수입니다.<br><br>
#                 <b style='color:#FBBF24;'>💡 해석 방법:</b> 100보다 낮으면 '소비 위축' 상태입니다. <b>더 진한 빨간색 동그라미</b>에 마우스를 올려 원인을 확인하세요.
#             </div>
#         </div>
#     """, unsafe_allow_html=True)
...
#     # 🚨 이벤트 유무에 따라 CCSI(빨간색 선) 마커 색상 및 크기 동적 할당
#     ccsi_colors = ['#7F1D1D' if e != "" else '#D97A7A' for e in df_filtered['event']] # 진한 빨강 vs 기본 빨강
#     ccsi_sizes = [12 if e != "" else 6 for e in df_filtered['event']]
    
#     fig2.add_trace(go.Scatter(
#         x=df_filtered[x_col], y=df_filtered['ccsi'], name='CCSI', mode='lines+markers',
#         line=dict(color='#D97A7A', width=3),
#         marker=dict(color=ccsi_colors, size=ccsi_sizes, line=dict(color='white', width=1)),
#         customdata=df_filtered['event'], hovertemplate="CCSI: %{y:.1f}%{customdata}<extra></extra>"
#     ))
#     fig2.add_hline(y=100, line_dash="dash", line_color="gray", annotation_text="낙관/비관 기준선(100)")
    
#     fig2.update_layout(
#         height=320, margin=dict(l=0, r=0, t=20, b=0), hovermode="x unified",
#         yaxis=dict(range=[85, 110]), showlegend=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'
#     )
...
# with st.container(border=True):
#     st.markdown("""
#         <div class="title-tooltip" style="margin-bottom: 5px;">
#             <p style='font-weight: 700; font-size: 15px; color: #1E293B; margin-bottom: 0;'>
#                 지표별 이탈 vs 유지 비교 (이커머스 누적 데이터) <span style='color: #94A3B8; font-size: 13px; font-weight: 400;'>ⓘ</span>
#             </p>
#             <div class="tooltip-text">
#                 <b style='color:#A78BFA;'>📊 그래프 설명:</b> 떠난 고객과 유지 고객의 행동 차이입니다.<br><br>
#                 <b style='color:#FBBF24;'>💡 해석 방법:</b> 막대 격차가 클수록 이탈의 핵심 원인입니다. 막대에 마우스를 올려 세부 해석을 확인하세요.
#             </div>
#         </div>
#     """, unsafe_allow_html=True)
...
#         customdata=df_churn['churn_event'], hovertemplate="<b>%{x} (이탈)</b><br>수치: %{y}<br>💡 %{customdata}<extra></extra>"
#     ))
#     fig3.add_trace(go.Bar(
#         name='유지 고객', x=df_churn['metric'], y=df_churn['retain'], marker_color='#4A6FA5',
#         customdata=df_churn['retain_event'], hovertemplate="<b>%{x} (유지)</b><br>수치: %{y}<br>💡 %{customdata}<extra></extra>"
#     ))
#     fig3.update_layout(
...
# # --- 8. 마스터 필터 연동 동적 인사이트 ---
# st.write("")
# st.markdown(f"<h3 style='font-size: 18px; font-weight: 800; color: #1E293B;'>💡 {global_year_filter} 핵심 인사이트 분석</h3>", unsafe_allow_html=True)
...
# for col, (color, title, desc) in zip([i1, i2, i3, i4], current_insights):
#     with col:
#         st.markdown(f"""
...
#                 <p style="margin: 0; font-size: 13px; font-weight: 500; color: #334155; line-height: 1.6;">{desc}</p>
#             </div>
#         """, unsafe_allow_html=True)
#         st.write("")
